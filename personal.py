...
def main():
    parser = argparse.ArgumentParser(description="Run personal Docker container with various mounts")
    parser.add_argument("--port", "-p", type=str, help="Port to expose")
    parser.add_argument("--image-name", "-i", type=str, default="personal", help="Docker image name (default: personal)")
    parser.add_argument("--start-cmd", "-c", type=str, default="xonsh", help="Start command (default: xonsh)")
    ...
    docker_cmd = [
        ...
        "--mount", f"type=bind,src={docker_clipboard_dir},dst=/clipboard",
    ]
